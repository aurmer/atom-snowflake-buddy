...
import { isValidHextail,isValidFlake,ALL_FLAKES_IN_STRING } from './flake-validation.js'
...
export default {
  watchFile: (textEditor) => {
    ...
    const hextailOpacity = (100 - atom.config.get(PACKAGE_NAME).hextailTransparency) / 100
    textEditor.decorateMarkerLayer(mkLay, { type: 'text', style: { opacity: hextailOpacity } })
    textBuffer.scanInRange(ALL_FLAKES_IN_STRING,
      textBuffer.getRange(),

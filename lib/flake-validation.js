...
const FLAKE_AT_END_OF_STRING = new RegExp(
  /(^|[^A-Za-z0-9-])([a-z][a-z-]*?-([a-f0-9]{5}))$/
);
export const ALL_FLAKES_IN_STRING = new RegExp(
  /(^|[^A-Za-z0-9-])([a-z][a-z-]*?-([a-f0-9]{5}))(?![A-Za-z0-9-])/g
);
const START_OF_A_VALID_FLAKE = new RegExp(
  /^(.*?[^_A-Za-z-0-9])?([a-z][a-z-0-9]*?)$/
);

export const isValidHextail = (hextail) => {
  // Hextail must include [a-f] and \d
  return (
    typeof hextail === "string" &&
...
export const getFlakeBeginningMatch = (editor, bufferPosition) => {
  ...
  return flakePrefixMatch ? flakePrefixMatch[2] : "";
};
...
assert.deepStrictEqual(
  getAllFlakes(
    "flake-7df0c,foo-ef1c5,aa-f2df0-bar-b5e2f[button-554c8].nav-998bf,invalid-f54eee\nnewline-f56c0\nsection-ccc85,invalid-hex-1111,invalid-hex-aaaa"
  ),
  [
    "flake-7df0c",
    "foo-ef1c5",

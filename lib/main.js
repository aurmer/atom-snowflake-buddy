...
import flakeDecorator from "./flake-decorator";
import {
  didJustGenerateHextail,
  getCharBeforeCursor,
  getActiveTextEditor,
} from "./util.js";
...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(
      atom.commands.add("atom-text-editor", {
        [`${PACKAGE_NAME}:generate-hextail`]: () =>
          this.handleGenerateHextail(),
      })
    );
    ...
    // tab change hook
    this.subscriptions.add(
      atom.workspace.observeActiveTextEditor(flakeProvider.newFileWatch)
    );
    this.subscriptions.add(
      atom.workspace.observeActiveTextEditor(flakeDecorator.watchFile)
    );
    this.subscriptions.add(
      atom.workspace.observeActiveTextEditor(this.addSubscriptionTextEditor)
    );
  },
      ...
      if (!textEditor.snowflakeIsHooked) {
        ...
        this.subscriptions.add(
          textEditor.onDidChangePath(
            flakeProvider.newFileWatch.bind(null, textEditor)
          )
        );
        // don't play nice with multi-cursor
        this.subscriptions.add(
          textEditor.onDidAddCursor(
            flakeProvider.cursorWatch.bind(null, textEditor)
          )
        );
        this.subscriptions.add(
        ...
        this.subscriptions.add(
          textEditor
            .getBuffer()
            .onDidChange(flakeDecorator.watchChanges.bind(null, textEditor))
        );
        // flake detection on typing
        this.subscriptions.add(
          textEditor.onDidInsertText(
            flakeProvider.bigBrotherWatch.bind(null, textEditor)
          )
        );
      }
    }
  ...
  handleGenerateHextail() {
    ...
    const isRerolling = didJustGenerateHextail(
      textEditor,
      this.hextailGenCheckpoint
    );
